page 52048977 "Loan Advance archives cloturés"
{ //GL2024  ID dans Nav 2009 : "39001504"
    Caption = 'Archives Avances Clôtures';
    ...
    SourceTableView = SORTING("No.") WHERE(Type = FILTER(Advance), Status = FILTER(Enclosed));
    ...
    layout
    {
        area(content)
        {
            ...
            group(Remboursement)
            {
                ...
                field("Total to repay3"; rec."Total to repay")
                {
                    ApplicationArea = all;
                    Editable = false;
                }
