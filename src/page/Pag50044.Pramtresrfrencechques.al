Page 50044 "Pramétres référence chèques"
{
    //   //>>>MBK:05/02/2010: Référence chèque

    PageType = List;
    SourceTable = "Référence Chèques";
    ApplicationArea = all;
    UsageCategory = Lists;
    Caption = 'Pramétres référence chèques';
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ShowCaption = false;
                field("N° ligne"; rec."N° ligne")
                {
                    ApplicationArea = all;
                    Editable = false;
                }
                field("Code banque"; rec."Code banque")
                {
                    ApplicationArea = all;
                    Editable = false;
                }
                field("Référence Chèques"; rec."Référence Chèques")
                {
                    ApplicationArea = all;
                }
                field("N° début"; rec."N° début")
                {
                    ApplicationArea = all;
                }
                field("N° fin"; rec."N° fin")
                {
                    ApplicationArea = all;
                }
                field("Dernier N° utilisé"; rec."Dernier N° utilisé")
                {
                    ApplicationArea = all;
                }
                field("Dernier date utilisé"; rec."Dernier date utilisé")
                {
                    ApplicationArea = all;
                }
                field("Date création"; rec."Date création")
                {
                    ApplicationArea = all;
                }
                field("Date début utilisation"; rec."Date début utilisation")
                {
                    ApplicationArea = all;
                }
                field("Date fin utilisation"; rec."Date fin utilisation")
                {
                    ApplicationArea = all;
                }
            }
        }
    }

    actions
    {
    }

    trigger OnInit()
    begin
        CurrPage.LookupMode := true;
    end;
}

