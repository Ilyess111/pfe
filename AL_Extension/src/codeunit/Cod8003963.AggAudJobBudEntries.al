Codeunit 8003963 "Agg. Aud. Job Bud. Entries"
{
    // #8127 ML 29/06/10
    // //NAVIBAT_STATS GESWAY 05/07/05 Statistic Aggregates Audit Job Budget Entries
    ...
    begin
        ...
        Entry.SetFilter("Entry Type", '%1|%2', Entry."entry type"::Initial, Entry."entry type"::Audit);
            ...
            with Entry do
                repeat
                    ...
                    if (Quantity <> 0) or ("Total Cost" <> 0) or ("Line Amount (LCY)" <> 0) or ("Gross Total Cost" <> 0) then begin
                        ...
                        StatisticAggregate1."Entry Type" := TextType[24];
                        case Type of
                            Type::Item:
                        ...
                        //#8127
                        if (SearchDim[24]) then begin
                            StatsExplorerTools.SearchDefaultDimension(StatisticAggregate1, 8004160, "Job No.");
                        end;
                        //#8127//
                        UpdateDefaultValues.Run(StatisticAggregate1);
                        if not StatsExplorerTools.SearchAndModifyAggregate(StatisticAggregate1, StatisticAggregate2, SearchDim[24], 24) then begin
