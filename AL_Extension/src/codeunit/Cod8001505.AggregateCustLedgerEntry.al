Codeunit 8001505 "Aggregate, Cust. Ledger Entry"
{
    //GL2024  ID dans Nav 2009 : "8001305"
    // #8144 ML 05/07/10
    // #6572 SD 25/03/10
    // //STATSEXPLORER STATSEXPLORER 01/10/01 Aggregate, Cust. Ledger Entry by Due Date or by Posting Date
    ...
    begin
        ...
        if EntryNo = 0 then
            ...
            if StatisticsSetup."Period total basis" <> StatisticsSetup."period total basis"::"According to every flow" then begin
                if WorkType[60] then begin
                    DueDate := true;
                    BasePeriod := StatisticsSetup."Period total basis";
                    SearchEntry;
                end;
                if WorkType[61] then begin
                    DueDate := false;
                    BasePeriod := StatisticsSetup."Period total basis";
                    SearchEntry;
                end;
                ...
                repeat
                    i := i + 1;
                    if PeriodBasis[60, i] then begin
                        DueDate := true;
                        BasePeriod := i - 1;
                        SearchEntry;
                    end;
                    if PeriodBasis[61, i] then begin
                        DueDate := false;
                        BasePeriod := i - 1;
                        SearchEntry;
                    end;
    ...
    begin
        ...
        if EntryNo = 0 then begin
            if DueDate then begin
                Entry.SetCurrentkey("Customer No.", "Customer Posting Group", "Global Dimension 1 Code",
                                     "Global Dimension 2 Code", "Currency Code", "Due Date");
                Entry.SetRange("Due Date", StartDate, EndDate);
            end else begin
                Entry.SetCurrentkey("Customer No.", "Customer Posting Group", "Global Dimension 1 Code",
            ...
            with Entry do
                repeat
                    ...
                    if (Amount <> 0) or ("Amount (LCY)" <> 0) then begin
                        ...
                        StatisticAggregate1."Source No." := "Customer No.";
                        StatisticAggregate1."Source Posting Group" := "Customer Posting Group";
                        ...
                        //#8144//
                        if ((StatisticAggregate1."Entry Type" = TextType[60]) and (SearchDim[60])) or
                           ((StatisticAggregate1."Entry Type" = TextType[61]) and (SearchDim[61])) then begin
