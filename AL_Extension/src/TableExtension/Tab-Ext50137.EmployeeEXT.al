TableExtension 50137 EmployeeEXT extends Employee
{
    ...
    fields
    {
        ...
        field(39001633; Affectation; Code[10])
        {
            Description = '//MBY ENDA';
            ...
            trigger OnValidate()
            begin
                Section.SetRange(Section, Affectation);
                if Section.FindFirst then begin
                    Service.SetRange(Service, Section.Service);
                    if Service.FindFirst then Validate("Type Affectation", Section.Service);
                    Chantier := Section.Chantier;
                    if Chantier = '' then Error(Text003);
                end;
                CalcFields("Deccription Affectation");
            end;
        }
        field(39001634; "Type Affectation"; Code[10])
        {
            Description = '//MBY ENDA';
            Editable = false;
            ...
            trigger OnValidate()
            begin
                CalcFields("Description Type Affectation");
            end;
        }
        field(39001635; "Description Type Affectation"; Text[50])
        {
            CalcFormula = lookup(Service.Description where(Service = field("Type Affectation")));
            Description = '//MBY ENDA';
            Editable = false;
            FieldClass = FlowField;
        }
        field(39001630; Chantier; Code[20])
        {
            TableRelation = Job;
        }
        field(39001631; "Deccription Affectation"; Text[50])
        {
        ...
        field(39001632; "Description Qualification"; Text[50])
        {
            CalcFormula = lookup(Qualification.Description where(Code = field(Qualification)));
            Editable = false;
            FieldClass = FlowField;
        }
        field(39001636; Qualification; Code[20])
        {
            TableRelation = Qualification;

            trigger OnValidate()
            begin
                RecQualification.SetRange(Code, rec.Qualification);
                if RecQualification.FindFirst then begin
                    Direction.SetRange(Code, RecQualification."Groupe Qualification");
                    if Direction.FindFirst then Validate("Groupe Qualification", RecQualification."Groupe Qualification");
                end;

                CalcFields("Description Qualification");
            end;
        }
        field(39001637; "Groupe Qualification"; Code[10])
        {
            Description = '//MBY ENDA';
            Editable = true;
            ...
            trigger OnValidate()
            begin
           //     CalcFields("Description Group Qualif");
            end;
        }
