//HS
xmlport 50100 "Disquette déclaration CNSS"
{
    ...
    //         {
    //             SourceTableView = SORTING("Social Security No.") ORDER(Ascending) WHERE(Collège = FILTER('<>KR*&<>SV*'), "Exclu De Dec Trim CNSS" = CONST(false));
    ...
    //                     LigneSalaireEnreg.SETFILTER(Month, FORMAT(PaieFrom.Month) + '..' + FORMAT(PaieTo.Month));
    ...
    //             {
    //                 field(PaieFrom; PaieFrom.Month)
    //                 {
    //                     ApplicationArea = all;
    //                     Caption = 'De';
    //                     OptionCaption = 'Janvier,Fevrier,Mars,Avril,Mai,Juin,Juillet,Aout,Septembre,Octobre,Novembre,Decembre';
    ...
    //     FILENAME := STRSUBSTNO('DS%1.%2%3', CNSSEmpl, Trim, AnnéeM);
