...
page 52049073 "Lignes planning projet B/F"
{
    AutoSplitKey = true;
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                // field("Qty. Invoiced"; Rec."Qty. Invoiced") { ApplicationArea = all; }
                ...
                field("Cumulative Executed Qty"; Rec."Cumulative Executed Qty")
                {
                    ApplicationArea = all;
                    Editable = false;
                    Style = Favorable;
                }
                field("Monthly Executed Qty"; Rec."Monthly Executed Qty")
                {
                    ApplicationArea = all;
                    Editable = false;
                    Style = Favorable;
    ...
    begin
        ...
        Rec.SetFilter("Line Type", '%1|%2', Rec."Line Type"::"Both Budget and Billable", Rec."Line Type"::Billable);
        Rec.FilterGroup := 0;
        rec.Setfilter("Date Filter", '%1..%2', 0D, calcdate('FM', today));
    end;
